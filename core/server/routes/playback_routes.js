...
function registerPlaybackRoutes(app, {
    ...
    app.get('/:conf/play_lazy/:service/:hash/:fileIdx', async (req, res) => {
        ...
        try {
            ...
            const lazyCacheKey = DownloadLinkCache.buildLazyKey(requestedService, item, { season: item.season, episode: item.episode });
            ...
            const cachedLazy = await DownloadLinkCache.get(Cache, lazyCacheKey, { logger, incrementMetric, scope: 'lazy' });
            ...
            if (streamData && streamData.url) {
                const finalStreamData = DownloadLinkCache.wrapMediaFlowUrl(config, streamData);
                await DownloadLinkCache.set(Cache, lazyCacheKey, finalStreamData, { logger, incrementMetric, scope: 'lazy' });
                await markPlayableResultAsCached(requestedService, item, finalStreamData, playbackMeta);
                incrementMetric('lazyPlay.success');
                recordDuration('lazyPlay.total', Date.now() - startedAt);
                recordProviderMetric(`lazy.${requestedService}`, true, Date.now() - startedAt);
                return res.redirect(finalStreamData.url);
            }
    ...
    app.get('/:conf/play_saved_cloud/:service/:torrentId/:fileId', async (req, res) => {
        ...
        try {
            ...
            const cacheKey = DownloadLinkCache.buildSavedCloudKey(requestedService, torrentId, fileId);
            const cached = await DownloadLinkCache.get(Cache, cacheKey, { logger, incrementMetric, scope: 'saved' });
            if (cached?.url) {
            ...
            const finalStreamData = DownloadLinkCache.wrapMediaFlowUrl(config, streamData);
            const finalUrl = finalStreamData.url;

            await DownloadLinkCache.set(Cache, cacheKey, finalStreamData, { logger, incrementMetric, scope: 'saved' });
