...
function applyStremioStreamCacheHeaders(res, payload, logger = null) {
    ...
    const streamCount = Array.isArray(payload?.streams) ? payload.streams.length : 0;
    const cacheProfile = String(payload?.cacheProfile || (streamCount > 0 ? 'positive_long' : 'empty_short')).trim();

    res.setHeader('Access-Control-Allow-Origin', '*');
    res.setHeader('X-Leviathan-Cache-Profile', cacheProfile);
    if (cacheMaxAge <= 0) {
        res.setHeader('Cache-Control', 'no-store, no-cache, must-revalidate, max-age=0');
        res.setHeader('Pragma', 'no-cache');
        res.setHeader('Expires', '0');
        if (logger && typeof logger.info === 'function') logger.info(`[CACHE HEADERS] streams=${streamCount} profile=${cacheProfile} maxAge=0 staleRevalidate=0 staleError=0`);
        return;
    }
    ...
    if (logger && typeof logger.info === 'function') logger.info(`[CACHE HEADERS] streams=${streamCount} profile=${cacheProfile} maxAge=${cacheMaxAge} staleRevalidate=${staleRevalidate} staleError=${staleError}`);
}
...
function registerStremioRoutes(app, {
    ...
    app.get('/:conf/stream/:type/:id.json', async (req, res) => {
        try {
            ...
            applyStremioStreamCacheHeaders(res, result, logger);
            ...
            applyStremioStreamCacheHeaders(res, fallback, logger);
