...
const { encodeConfigObject, getConfigSecurityStatus } = require('../../security/config_crypto');
const { MAX_CONFIG_LENGTH } = require('../../config/schema');
...
function registerApiRoutes(app, {
    ...
    app.post('/api/config/encode', (req, res) => {
        res.setHeader('Cache-Control', 'no-store');
        res.setHeader('Pragma', 'no-cache');

        try {
            const config = req.body && typeof req.body === 'object' && !Array.isArray(req.body) ? req.body : {};
            const json = JSON.stringify(config);
            if (json.length > MAX_CONFIG_LENGTH) {
                return res.status(413).json({ ok: false, code: 'config_too_large', message: `Config troppo grande (${json.length})` });
            }

            const conf = encodeConfigObject(config);
            return res.json({
                ok: true,
                encrypted: String(conf || '').startsWith('enc:v1:'),
                conf,
                manifestPath: `/${conf}/manifest.json`,
                security: getConfigSecurityStatus()
            });
        } catch (error) {
            return res.status(400).json({ ok: false, code: 'encode_failed', message: 'Impossibile cifrare la configurazione.' });
        }
    });

    app.get('/api/security/status', telemetryAuthMiddleware, (req, res) => {
        res.setHeader('Cache-Control', 'no-store');
        res.json(getConfigSecurityStatus());
    });
