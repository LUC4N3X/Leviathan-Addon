...
function hasAntiBotBody(body = '') {
    return /cloudflare|cf-chl|captcha|attention required|verify you are human|just a moment|access denied|ddos-guard|checking your browser/i.test(String(body || ''));
}
...
function classifyProviderFailure({ error = null, response = null, rawResults = [], recipe = {}, context = {}, request = null } = {}) {
    ...
    if (error) {
        ...
        if (status === 403 || status === 401 || hasAntiBotBody(body)) {
            return { type: 'blocked', reason: status === 403 ? 'http_403_blocked' : 'anti_bot_blocked', status, recoverable: false, canFallback: true, isError: true };
        }
        if (status === 429) {
    ...
    if (status === 403 || status === 401 || hasAntiBotBody(body)) {
        return { type: 'blocked', reason: status === 403 ? 'http_403_blocked' : 'anti_bot_blocked', status, recoverable: false, canFallback: true, isError: false };
    }
