...
function classifyProviderFailure({ error = null, response = null, rawResults = [], recipe = {}, context = {}, request = null } = {}) {
    ...
    if (error) {
        ...
        if (status === 403 || status === 401 || antiBotBlock) {
            return { type: 'blocked', reason: status === 403 ? 'http_403_blocked' : blockReason, status, recoverable: false, canFallback: true, isError: true, details: blockDetails };
        }
        if (status === 429) {
    ...
    if (status === 403 || status === 401 || antiBotBlock) {
        return { type: 'blocked', reason: status === 403 ? 'http_403_blocked' : blockReason, status, recoverable: false, canFallback: true, isError: false, details: blockDetails };
    }
