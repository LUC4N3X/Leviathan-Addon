...
async function ensureDatabaseOptimizations(pool) {
  ...
  const statements = [
    ...
    `ALTER TABLE subtitles ADD COLUMN IF NOT EXISTS subtitle_key TEXT`,
    `ALTER TABLE subtitles ADD COLUMN IF NOT EXISTS info_hash TEXT`,
    `ALTER TABLE subtitles ADD COLUMN IF NOT EXISTS info_hash_norm TEXT`,
    `ALTER TABLE subtitles ADD COLUMN IF NOT EXISTS file_index INTEGER`,
    `ALTER TABLE subtitles ADD COLUMN IF NOT EXISTS file_index_norm INTEGER DEFAULT -1`,
    `ALTER TABLE subtitles ADD COLUMN IF NOT EXISTS lang TEXT`,
    `ALTER TABLE subtitles ADD COLUMN IF NOT EXISTS title TEXT`,
    `ALTER TABLE subtitles ADD COLUMN IF NOT EXISTS size BIGINT DEFAULT 0`,
    `ALTER TABLE subtitles ADD COLUMN IF NOT EXISTS url TEXT`,
    `ALTER TABLE subtitles ADD COLUMN IF NOT EXISTS source TEXT`,
    `ALTER TABLE subtitles ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ DEFAULT NOW()`,
    `ALTER TABLE subtitles ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT NOW()`,
    ...
    `UPDATE subtitles SET info_hash_norm = LOWER(TRIM(info_hash)) WHERE info_hash IS NOT NULL AND (info_hash_norm IS NULL OR info_hash_norm <> LOWER(TRIM(info_hash)))`,
    `UPDATE subtitles SET file_index_norm = COALESCE(file_index, -1) WHERE file_index_norm IS DISTINCT FROM COALESCE(file_index, -1)`,
    `UPDATE subtitles SET subtitle_key = md5(COALESCE(info_hash_norm, LOWER(TRIM(info_hash)), '') || ':' || COALESCE(file_index_norm, COALESCE(file_index, -1))::text || ':' || COALESCE(lang, '') || ':' || COALESCE(title, '') || ':' || COALESCE(url, '')) WHERE subtitle_key IS NULL OR subtitle_key = ''`,
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS uq_subtitles_key ON subtitles (subtitle_key)`,
    `CREATE INDEX IF NOT EXISTS idx_subtitles_hash_file ON subtitles (info_hash_norm, file_index_norm)`,
    `CREATE INDEX IF NOT EXISTS idx_subtitles_lang ON subtitles (lang, info_hash_norm, file_index_norm)`,
