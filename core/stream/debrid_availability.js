...
function createDebridAvailabilityTools({ Cache, logger, LIMITERS, CONFIG, incrementMetric, isSeasonPack, getMetaDbLookupKey }) {
    ...
    async function hydrateRdForegroundAvailability(items, apiKey, options = {}) {
        ...
        let availabilityFileCacheHits = 0;
        for (const item of allUnknownCandidates) {
            try {
                const scopedCached = await DebridAvailabilityCache.get(Cache, 'rd', item);
                if (scopedCached?.payload && DebridAvailabilityCache.applyPayloadToItem(item, scopedCached.payload, 'rd')) {
                    availabilityFileCacheHits += 1;
                    continue;
                }
            } catch (_) {}
        ...
        if (availabilityFileCacheHits > 0) logger.info(`[DEBRID CACHE] availability service=rd hits=${availabilityFileCacheHits} keyScope=infoHash:fileIdx`);
            ...
            for (const item of unknownCandidates) {
                ...
                if (typeof Cache.cacheAvailability === 'function') {
                    ...
                    if (availabilityCacheKey) {
                        ...
                        await DebridAvailabilityCache.set(Cache, 'rd', item, { ...statePayload, fileIdx: item?.fileIdx, file_size: result?.file_size }, availabilityTtl);
                    }
                }
            }
            ...
            if (Array.isArray(deferred) && deferred.length > 0) {
                for (const item of deferred) {
                    ...
                    if (typeof Cache.cacheAvailability === 'function') {
                        const availabilityCacheKey = getAvailabilityCacheKey('rd', item?.hash);
                        if (availabilityCacheKey) {
                            await Cache.cacheAvailability(availabilityCacheKey, buildAvailabilityCachePayload({ state: 'probing', cached: null, failures: item?._dbFailures || 0 }, item, null), AVAILABILITY_CACHE_PROBING_TTL);
                            await DebridAvailabilityCache.set(Cache, 'rd', item, { state: 'probing', cached: null, failures: item?._dbFailures || 0, fileIdx: item?.fileIdx }, AVAILABILITY_CACHE_PROBING_TTL);
                        }
                    }
                }
    ...
    async function persistResolvedDebridAvailability(meta, item, streamData, service, reason = 'direct_resolve') {
        ...
        try {
            ...
            await DebridAvailabilityCache.set(Cache, normalizedService, item, {
                state: 'cached',
                cached: true,
                fileIdx: Number.isInteger(parsedFileIndex) && parsedFileIndex >= 0 ? parsedFileIndex : item?.fileIdx,
                file_size: Number.isFinite(parsedFileSize) && parsedFileSize > 0 ? parsedFileSize : Number(item?._size || item?.sizeBytes || 0)
            }, AVAILABILITY_CACHE_HIT_TTL).catch(() => false);
