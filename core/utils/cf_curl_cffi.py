...
DOCUMENT_ACCEPT_CHROMIUM = "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8"
DOCUMENT_ACCEPT_FIREFOX = "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8"
DOCUMENT_ACCEPT_SAFARI = "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8"
JSON_ACCEPT = "application/json, text/plain, */*"
SCRIPT_ACCEPT = "application/javascript,text/javascript,*/*;q=0.8"
MEDIA_ACCEPT = "*/*"
HLS_ACCEPT = "application/vnd.apple.mpegurl, application/x-mpegURL, */*"
DASH_ACCEPT = "application/dash+xml, */*"
...
def infer_request_context(url: str, headers: Dict[str, Any]) -> str:
    dest = str(header_get(headers, "sec-fetch-dest") or "").lower()
    accept = str(header_get(headers, "accept") or "").lower()
    path = urlparse(str(url or "")).path.lower()
    if dest == "iframe":
        return "iframe"
    if dest == "script" or path.endswith(".js"):
        return "script"
    if dest == "video":
        return "media"
    if "mpegurl" in accept or path.endswith(".m3u8") or "/playlist/" in path:
        return "playlist"
    if "dash+xml" in accept or path.endswith(".mpd"):
        return "dash"
    if dest == "empty" or "json" in accept or path.endswith(".json") or "/api/" in path:
        return "api"
    if re.search(r"\.(ts|m4s|mp4|mkv|avi|mov|webm|m4v|mp3|m4a|aac|vtt|srt|key)$", path, re.I):
        return "media"
    return "document"


def context_accept(context: str, impersonate: str, user_agent: str) -> str:
    if context == "api":
        return JSON_ACCEPT
    if context == "script":
    ...
    if context == "media":
        return MEDIA_ACCEPT
    if is_firefox_based(impersonate, user_agent):
        return DOCUMENT_ACCEPT_FIREFOX
    if is_safari_based(impersonate, user_agent):
        return DOCUMENT_ACCEPT_SAFARI
    return DOCUMENT_ACCEPT_CHROMIUM


def apply_context_headers(ordered: Dict[str, str], context: str, sec_fetch_site: str, impersonate: str, user_agent: str) -> None:
    put_header(ordered, "Accept", context_accept(context, impersonate, user_agent))
    if context == "api":
        put_header(ordered, "Sec-Fetch-Site", sec_fetch_site)
        put_header(ordered, "Sec-Fetch-Mode", "cors")
        put_header(ordered, "Sec-Fetch-Dest", "empty")
        return
    if context == "script":
        put_header(ordered, "Sec-Fetch-Site", sec_fetch_site)
        put_header(ordered, "Sec-Fetch-Mode", "no-cors")
        put_header(ordered, "Sec-Fetch-Dest", "script")
        return
    if context in {"playlist", "dash", "media"}:
        put_header(ordered, "Sec-Fetch-Site", sec_fetch_site)
        put_header(ordered, "Sec-Fetch-Mode", "cors")
        put_header(ordered, "Sec-Fetch-Dest", "video" if context == "media" else "empty")
        return
    if context == "iframe":
        put_header(ordered, "Upgrade-Insecure-Requests", "1")
        put_header(ordered, "Sec-Fetch-Site", sec_fetch_site)
        put_header(ordered, "Sec-Fetch-Mode", "navigate")
        put_header(ordered, "Sec-Fetch-Dest", "iframe")
        return
    put_header(ordered, "Upgrade-Insecure-Requests", "1")
    put_header(ordered, "Sec-Fetch-Site", sec_fetch_site)
    put_header(ordered, "Sec-Fetch-Mode", "navigate")
...
def browser_ordered_headers(
    ...
    *,
    ...
    context = infer_request_context(url, custom)
    ...
    apply_context_headers(ordered, context, sec_fetch_site, impersonate, ua)
    ...
    if is_chromium_based(impersonate, ua) and context == "document":
    ...
    if not is_chromium_based(impersonate, header_get(ordered, "user-agent")):
        ...
        ordered.pop("Priority", None)

    if infer_request_context(url, ordered) != "document":
        ordered.pop("Sec-Fetch-User", None)
        ordered.pop("Upgrade-Insecure-Requests", None)
        if header_key(ordered, "priority"):
            ordered.pop(header_key(ordered, "priority"), None)
