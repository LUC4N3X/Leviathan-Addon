...
function compactCacheLogValue(value, maxLength = 140) {
    const text = String(value ?? '').replace(/\s+/g, ' ').trim();
    if (!text) return 'empty';
    return text.length > maxLength ? `${text.slice(0, maxLength)}…` : text;
}

function normalizeRawCacheIdentity(provider, id) {
    const providerKey = String(provider || 'UnknownProvider').trim() || 'UnknownProvider';
    const idKey = String(id ?? '').trim() || 'unknown';
    return { providerKey, idKey };
}

function buildRawFetchInflightKey(provider, id, options = {}) {
    const mode = options?.bypassCache === true ? 'bypass' : 'normal';
...
const Cache = {
    ...
    fetchWithCache: async (provider, id, ttl = RAW_PROVIDER_CACHE_TTL, fetcherFunc, options = {}) => {
        const { providerKey, idKey } = normalizeRawCacheIdentity(provider, id);
        const cacheOnly = options?.cacheOnly === true;
        // cacheOnly deve vincere su bypassCache: in modalità cached-only non deve mai partire I/O esterno.
        ...
        if (!bypassCache) {
            ...
            incrementMetric('cache.raw.bypass');
        }

        if (typeof fetcherFunc !== 'function') {
            logger.warn(`[CACHE] Provider fetcher missing | provider=${compactCacheLogValue(providerKey)} | id=${compactCacheLogValue(idKey)}`);
            return [];
        }
        ...
        return withSharedPromise(sharedFetchInflight, inflightKey, async () => {
            ...
            try {
                ...
                const normalized = normalizeProviderFetchPayload(freshData);
                Cache.setRaw(providerKey, idKey, normalized, normalized.length > 0 ? effectiveTtl : emptyTtl);
                return normalized;
            } catch (error) {
