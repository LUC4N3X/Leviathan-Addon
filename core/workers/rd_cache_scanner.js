...
function createRdHttpClient(token) {
    return axios.create({
...
async function probeHashSlow(http, hash) {
    let torrentId = null;
    try {
        ...
        if (torrentId) {
            try { await http.delete(`/torrents/delete/${torrentId}`); } catch (_) {}
        }
    }
}
...
function getRdCacheScannerStatus() {
    return { ...scannerState };
}
...
function startRdCacheScanner({ dbHelper, logger = console, onBatchUpdated = null } = {}) {
    ...
    (async () => {
        if (typeof dbHelper.normalizePendingRdCacheState === 'function') {
            ...
            logger.info?.(`[RD SCANNER] Bootstrap normalize | applied=${normalized?.applied ? 'true' : 'false'} | updated=${normalized?.updated || 0} | reason=${normalized?.reason || 'n/a'}`);
        }
        ...
        while (true) {
            try {
                ...
                scannerState.lastActivityAt = new Date().toISOString();
                scannerState.lastBatchSize = Array.isArray(batch) ? batch.length : 0;
                if (!Array.isArray(batch) || batch.length === 0) {
                    logger.info?.(`[RD SCANNER] Nessun hash pronto | sleep=${idleMs}ms`);
                    await sleep(idleMs);
                    continue;
                }

                logger.info?.(`[RD SCANNER] Batch preso dal DB | elementi=${batch.length}`);
                const results = [];
                for (const item of batch) {
                    const hash = String(item?.hash || '').trim().toLowerCase();
                    if (!hash) continue;
                    scannerState.currentHash = hash;
                    scannerState.currentTitle = String(item?.title || '').slice(0, 180) || null;
                    logger.info?.(`[RD SCANNER] Controllo hash=${hash} | title=${scannerState.currentTitle || 'n/a'}`);
                    const outcome = await probeHashSlow(http, hash);
                    scannerState.lastOutcome = { ...outcome, at: new Date().toISOString() };
                    scannerState.totalScanned += 1;
                    logger.info?.(`[RD SCANNER] Esito hash=${hash} | cached=${outcome.cached} | reason=${outcome.reason || 'n/a'} | file=${outcome.rd_file_index ?? 'n/a'} | size=${outcome.rd_file_size ?? 'n/a'} | next_hours=${outcome.next_hours}`);
                ...
                if (results.length > 0) {
                    ...
                    scannerState.totalUpdated += Number(updated || 0);
                    scannerState.lastBatchAt = new Date().toISOString();
                    scannerState.currentHash = null;
                ...
                scannerState.lastError = String(err?.message || err);
                scannerState.lastActivityAt = new Date().toISOString();
                logger.error?.(`[RD SCANNER] Errore loop: ${err.message}`);
                await sleep(idleMs);
            }
        }
    })().catch((err) => {
        ...
        scannerState.lastError = String(err?.message || err);
        logger.error?.(`[RD SCANNER] Fatal: ${err.message}`);
    });

    return { enabled: true, started: true, reason: 'started' };
}

module.exports = { startRdCacheScanner, getRdCacheScannerStatus };
