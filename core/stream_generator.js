...
const backgroundScraperQueue = require('./background_scraper_queue');
const DownloadLinkCache = require('./playback/download_link_cache');
const RawStreamCache = require('./cache/raw_stream_cache');
const { applyTorrentSeedHealthRanking } = require('./ranking/torrent_seed_health_ranker');
const { applySmartStreamDedup } = require('./ranking/smart_stream_deduper');
const { applyBitrateRuntimeRanking } = require('./ranking/bitrate_runtime_ranker');
const { applyNormalizedProxyHeadersToStream } = require('./proxy/proxy_header_normalizer');
const { enrichTorrentTrackers } = require('./torrent/tracker_enricher');
const DebridAvailabilityCache = require('./debrid/debrid_availability_cache');
const {
...
} = require('./provider_execution_modes');
...
function getTorrentProviderNames() {
    const scraperNames = SCRAPER_MODULES
        .map((scraper) => scraper?.name || 'ScraperModule')
        .filter(Boolean);
    return ['RemoteIndexer', 'ExternalAddons', 'ScraperModule', ...scraperNames];
}

function providerAllowedInExecutionContext(providerName, config = {}, context = 'live', fallback = 'both') {
    const mode = getProviderExecutionMode(providerName, config, fallback);
    const normalizedContext = String(context || 'live').toLowerCase();
    return normalizedContext === 'background' ? providerAllowsBackground(mode) : providerAllowsLive(mode);
}

function logProviderModeSkip(providerName, mode, context, reason = 'mode') {
    logger.info(`[PROVIDER MODE] skip provider=${providerName} mode=${mode} context=${context} reason=${reason}`);
}
...
function parseFalsey(value) {
    return /^(?:0|false|no|off)$/i.test(String(value || '').trim());
}

function getBackgroundScraperMinResults(config = {}) {
    const raw = config?.filters?.backgroundScraperMinResults ?? process.env.BACKGROUND_SCRAPER_MIN_RESULTS ?? '6';
    return Math.max(0, Math.min(50, parseInt(raw, 10) || 6));
}

function shouldUseBackgroundScraper(config = {}, sourceModeFlags = {}) {
    const explicit = config?.filters?.backgroundScraper;
    if (parseFalsey(explicit)) return false;
    if (sourceModeFlags.dbOnlyMode || sourceModeFlags.cacheOnlyMode) return false;
    return backgroundScraperQueue.isEnabled();
}

function getBackgroundScrapeReason({ config = {}, cleanResults = [], finalRanked = [], finalStreams = [], sourceModeFlags = {}, torrentPipelineEnabled = true } = {}) {
    if (torrentPipelineEnabled !== true) return null;
    if (!shouldUseBackgroundScraper(config, sourceModeFlags)) return null;
    if (parseTruthy(process.env.BACKGROUND_SCRAPER_ALWAYS) || parseTruthy(config?.filters?.backgroundScraperAlways)) return 'forced';
    const providerNames = getTorrentProviderNames();
    if (hasBackgroundOnlyProvider(config, providerNames)) return 'background_only_provider';
    const minResults = getBackgroundScraperMinResults(config);
    if ((Array.isArray(finalRanked) ? finalRanked.length : 0) < minResults) return 'low_ranked_results';
    if ((Array.isArray(cleanResults) ? cleanResults.length : 0) === 0) return 'empty_torrent_pool';
    if ((Array.isArray(finalStreams) ? finalStreams.length : 0) === 0) return 'empty_streams';
    if (hasBackgroundCapableProvider(config, providerNames) && parseTruthy(config?.filters?.backgroundScraper)) return 'explicit_background';
    return null;
}

function buildBackgroundScrapeKey({ type, finalId, meta = {}, config = {}, langMode = 'ita' } = {}) {
    const safeConfig = {
        service: config?.service || '',
        filters: config?.filters || {},
        providerModes: buildProviderModeSignature(config, getTorrentProviderNames())
    };
    const configSig = crypto.createHash('sha1').update(JSON.stringify(safeConfig)).digest('hex').slice(0, 12);
    return [
        'title',
        String(type || '').toLowerCase(),
        String(finalId || meta?.imdb_id || meta?.id || 'unknown').replace(/[^a-zA-Z0-9:_-]+/g, '_'),
        Number(meta?.season || 0) || 0,
        Number(meta?.episode || 0) || 0,
        String(langMode || 'ita').toLowerCase(),
        configSig
    ].join(':');
}

function scheduleBackgroundTitleScrape(options = {}) {
    const reason = getBackgroundScrapeReason(options);
    if (!reason) return false;

    const { type, finalId, tmdbIdLookup, meta, config, sourceModeFlags, langMode, aggressiveFilter, userTmdbKey, seedResults = [] } = options;
    const key = buildBackgroundScrapeKey({ type, finalId, meta, config, langMode });

    const outcome = backgroundScraperQueue.enqueue({
        key,
        label: `background-title-search:${meta?.title || finalId || 'unknown'}`,
        reason,
        priority: reason === 'empty_streams' || reason === 'empty_torrent_pool' ? 10 : 5,
        run: async () => {
            const bgFlags = {
                ...(sourceModeFlags || getSourceModeFlags(config?.filters || {})),
                dbOnlyMode: false,
                cacheOnlyMode: false,
                liveOnlyMode: false,
                useLocalDb: true,
            ...
            const bgResults = await fetchTitleCandidatePool({
                type,
                finalId,
                tmdbIdLookup,
                meta,
                config,
                dbOnlyMode: false,
                sourceModeFlags: bgFlags,
                torrentPipelineEnabled: true,
                langMode,
                aggressiveFilter,
                userTmdbKey,
                seedResults,
            ...
            let normalized = await normalizeCandidateResults([...(Array.isArray(seedResults) ? seedResults : []), ...bgResults].filter(aggressiveFilter), meta, type);
            normalized = runFilterStage(normalized, meta, config?.filters || {}, {
                applyPackKnowledge,
                applyConfiguredTorrentFilters
            });
            if (normalized.length > 0) saveResultsToDbBackground(meta, normalized, config, type);
            logger.info(`[BG-SCRAPER] saved torrents=${normalized.length} imdb=${meta?.imdb_id || 'n/a'} reason=${reason}`);
            return { results: normalized.length };
        }
    }, { logger, incrementMetric, recordDuration });
...
const PRELOAD_TOP_STREAMS_ENABLED = String(process.env.PRELOAD_TOP_STREAMS_ENABLED || 'true').toLowerCase() !== 'false';
const PRELOAD_TOP_STREAMS_MAX = Math.max(0, Math.min(3, parseInt(process.env.PRELOAD_TOP_STREAMS_MAX || process.env.LAZY_WARMUP_TOP || '2', 10) || 2));
const PRELOAD_TOP_STREAMS_SCAN_MAX = Math.max(PRELOAD_TOP_STREAMS_MAX, Math.min(8, parseInt(process.env.PRELOAD_TOP_STREAMS_SCAN_MAX || '4', 10) || 4));
const PRELOAD_TOP_STREAMS_TIMEOUT_MS = Math.max(1500, Math.min(15000, parseInt(process.env.PRELOAD_TOP_STREAMS_TIMEOUT_MS || '5000', 10) || 5000));
const PRELOAD_TOP_STREAMS_COOLDOWN_MS = Math.max(60 * 1000, Math.min(60 * 60 * 1000, parseInt(process.env.PRELOAD_TOP_STREAMS_COOLDOWN_MS || String(15 * 60 * 1000), 10) || 15 * 60 * 1000));
const PRELOAD_TOP_STREAMS_REQUIRE_CACHED = String(process.env.PRELOAD_TOP_STREAMS_REQUIRE_CACHED || 'true').toLowerCase() !== 'false';
const PRELOAD_RD_REAL_CALL_MAX = Math.max(0, Math.min(1, parseInt(process.env.PRELOAD_RD_REAL_CALL_MAX || '1', 10) || 1));
const PRELOAD_TB_REAL_CALL_MAX = Math.max(0, Math.min(2, parseInt(process.env.PRELOAD_TB_REAL_CALL_MAX || '2', 10) || 2));
const PRELOAD_OTHER_REAL_CALL_MAX = Math.max(0, Math.min(1, parseInt(process.env.PRELOAD_OTHER_REAL_CALL_MAX || '1', 10) || 1));
const preloadCooldown = new Map();
...
function buildTitleSearchPipelineKey(meta, type, langMode, dbOnlyMode = false, filters = {}) {
    ...
    const payload = {
        ...
        filters: {
            ...
            providerModes: filters?.providerModeSignature || filters?.providerModes || filters?.providerExecutionModes || null,
...
function normalizeFinalProxyHeaders(streams = [], config = {}) {
    if (!Array.isArray(streams) || streams.length === 0) return streams;
    let changed = 0;
    const normalized = streams.map((stream) => {
        const hasProxyHeaders = Boolean(stream?.behaviorHints?.proxyHeaders?.request || stream?.headers);
        const hasUrlCredentials = /^https?:\/\/[^\s/@]+:[^\s/@]+@/i.test(String(stream?.url || ''));
        if (!hasProxyHeaders && !hasUrlCredentials) return stream;
        const next = applyNormalizedProxyHeadersToStream(stream, { config });
        if (next !== stream && next?._proxyHeadersNormalized) changed += 1;
        return next;
    });
    if (changed > 0) {
        logger.info('[PROXY HEADERS] normalized streams=' + changed + '/' + streams.length);
        incrementMetric('proxyHeaders.normalized', changed);
    }
    return normalized;
}
...
async function normalizeCandidateResults(items, meta = {}, type = '') {
    let normalized = deduplicateResults(Array.isArray(items) ? items : []);
    normalized = enrichTorrentTrackers(normalized, meta, type, { logger });
...
async function resolveTorboxRankedList(rankedList, apiKey) {
    ...
    await DebridAvailabilityCache.hydrateItems(Cache, 'tb', sourceRanked, { logger }).catch(() => sourceRanked);
    ...
    for (const checkLimit of progressiveWindows) {
        ...
        const toCheck = candidates.filter((item) => !(item?._tbCached === true || item?.tbCached === true));

        logger.info('📦 [TB CHECK] Scansiono ' + toCheck.length + '/' + candidates.length + ' torrent alla ricerca di file video reali...');
        const cacheResults = toCheck.length > 0
            ? await LIMITERS.tbResolve.schedule(() => TbCache.checkCacheSync(toCheck, apiKey, dbHelper, checkLimit))
            : {};
        verifiedList = [];
        ...
        for (const item of candidates) {
            ...
            if (item?._tbCached === true || item?.tbCached === true || (result && result.cached === true)) {
                item._tbCached = true;
                item.tbCached = true;
                if (result?.file_size) {
                ...
                await DebridAvailabilityCache.set(Cache, 'tb', item, {
                    state: 'cached',
                    cached: true,
                    fileIdx: item?.fileIdx,
                    file_size: result?.file_size || item?._size || item?.sizeBytes || 0
                }).catch(() => false);
...
function getPreloadCooldownKey(service, item = {}, meta = {}) {
    const hash = String(item?.hash || item?.infoHash || '').toUpperCase();
    if (!hash) return null;
    const fileIdx = (item.fileIdx !== undefined && item.fileIdx !== null && !Number.isNaN(Number(item.fileIdx))) ? Number(item.fileIdx) : -1;
    const season = Number(meta?.season || item?.season || 0) || 0;
    const episode = Number(meta?.episode || item?.episode || 0) || 0;
    return `${service}:${hash}:${fileIdx}:${season}:${episode}`;
}

function isPreloadCooldownActive(service, item, meta) {
    const key = getPreloadCooldownKey(service, item, meta);
    if (!key) return true;
    const expiresAt = Number(preloadCooldown.get(key) || 0) || 0;
    if (expiresAt > Date.now()) return true;
    preloadCooldown.delete(key);
    return false;
}

function markPreloadCooldown(service, item, meta) {
    const key = getPreloadCooldownKey(service, item, meta);
    if (!key) return;
    preloadCooldown.set(key, Date.now() + PRELOAD_TOP_STREAMS_COOLDOWN_MS);
    if (preloadCooldown.size > 5000) {
        const now = Date.now();
        for (const [entryKey, expiresAt] of preloadCooldown) {
            if (Number(expiresAt || 0) <= now || preloadCooldown.size > 4500) preloadCooldown.delete(entryKey);
        }
    }
}

function isKnownCachedForSafePreload(service, item = {}) {
    if (!PRELOAD_TOP_STREAMS_REQUIRE_CACHED) return true;
    const normalizedService = getNormalizedDebridService(service);
    const state = String(item?._rdCacheState || item?.rdCacheState || item?.cacheState || '').toLowerCase();
    if (normalizedService === 'tb') return Boolean(item?._tbCached === true || item?.tbCached === true || state === 'cached');
    if (normalizedService === 'rd') {
        return Boolean(
            item?._dbCachedRd === true ||
            item?.cached_rd === true ||
            state === 'cached' ||
            item?.directUrl || item?._externalDirectUrl || item?.externalDirectUrl
        );
    }
    return false;
}

function getPreloadRealCallLimit(service) {
    const normalized = getNormalizedDebridService(service);
    if (normalized === 'rd') return PRELOAD_RD_REAL_CALL_MAX;
    if (normalized === 'tb') return PRELOAD_TB_REAL_CALL_MAX;
    return PRELOAD_OTHER_REAL_CALL_MAX;
}
...
function warmupLazyStreamsInBackground(config, items, meta) {
    ...
    if (!PRELOAD_TOP_STREAMS_ENABLED || !apiKey || !Array.isArray(items) || items.length === 0) return;
    const requestedWarmups = parseInt(config?.filters?.warmupTop ?? process.env.LAZY_WARMUP_TOP ?? String(PRELOAD_TOP_STREAMS_MAX), 10) || PRELOAD_TOP_STREAMS_MAX;
    const maxWarmups = Math.max(0, Math.min(PRELOAD_TOP_STREAMS_MAX, requestedWarmups));
    const scanLimit = Math.max(maxWarmups, Math.min(PRELOAD_TOP_STREAMS_SCAN_MAX, Number(items.length || 0)));
    const realCallLimit = Math.max(0, Math.min(maxWarmups, getPreloadRealCallLimit(service)));
    if (maxWarmups <= 0 || scanLimit <= 0 || realCallLimit <= 0) return;
    if (streamInflight.size >= LAZY_WARMUP_LOAD_THRESHOLD) {
        incrementMetric('preload.skippedLoad', Math.min(items.length, scanLimit));
        logger.info(`[PRELOAD] skip | reason=load inflight=${streamInflight.size} threshold=${LAZY_WARMUP_LOAD_THRESHOLD}`);
        return;
    }
    ...
    for (const item of items) {
        ...
        if (!isKnownCachedForSafePreload(service, item)) {
            incrementMetric('preload.skippedUncached');
            continue;
        }
        if (isPreloadCooldownActive(service, item, meta)) {
            incrementMetric('preload.skippedCooldown');
            continue;
        }
    ...
    if (candidates.length === 0) {
        logger.info(`[PRELOAD] skip | reason=no_safe_candidates service=${String(service || '').toUpperCase()} scan=${scanLimit} max=${maxWarmups} realLimit=${realCallLimit} requireCached=${PRELOAD_TOP_STREAMS_REQUIRE_CACHED}`);
        return;
    }

    logger.info(`[PRELOAD] adaptive scheduled scan=${candidates.length} max=${maxWarmups} realLimit=${realCallLimit} service=${String(service || '').toUpperCase()} timeout=${PRELOAD_TOP_STREAMS_TIMEOUT_MS}ms cooldown=${Math.round(PRELOAD_TOP_STREAMS_COOLDOWN_MS / 1000)}s requireCached=${PRELOAD_TOP_STREAMS_REQUIRE_CACHED}`);
    ...
    LIMITERS.lazyWarmup.schedule(async () => {
        ...
        let cacheHits = 0;
        for (const item of candidates) {
            if (prepared >= maxWarmups) break;
            const lazyCacheKey = DownloadLinkCache.buildLazyKey(service, item, meta);
            const cached = await DownloadLinkCache.get(Cache, lazyCacheKey, { logger, incrementMetric, scope: 'preload' });
            if (cached?.url) {
                incrementMetric('preload.cacheHit');
                cacheHits += 1;
                prepared += 1;
                markPreloadCooldown(service, item, meta);
                continue;
            }
            if (realCalls >= realCallLimit) {
                incrementMetric('preload.skippedRealLimit');
                continue;
            }
            markPreloadCooldown(service, item, meta);
            realCalls += 1;
            const startedAt = Date.now();
            try {
                const streamData = await withTimeout(resolveLazyStreamData(service, apiKey, item, meta), PRELOAD_TOP_STREAMS_TIMEOUT_MS, `preload.${service}`);
                if (streamData?.url) {
                    await DownloadLinkCache.set(Cache, lazyCacheKey, streamData, { logger, incrementMetric, scope: 'preload' });
                    prepared += 1;
                    incrementMetric('preload.success');
                    logger.info(`[PRELOAD] cached service=${String(service).toUpperCase()} hash=${String(item.hash || '').slice(0, 12)} fileIdx=${item.fileIdx ?? -1} ms=${Date.now() - startedAt}`);
                } else {
                    incrementMetric('preload.empty');
                }
                recordProviderMetric(`preload.${service}`, true, Date.now() - startedAt);
            } catch (err) {
                incrementMetric('preload.fail');
                recordProviderMetric(`preload.${service}`, false, Date.now() - startedAt, { timeout: /timeout/i.test(String(err?.message || '')), error: err?.message || err });
                logger.info(`[PRELOAD] fail service=${String(service).toUpperCase()} hash=${String(item.hash || '').slice(0, 12)} reason=${err?.message || err}`);
            }
        }
        logger.info(`[PRELOAD] adaptive done service=${String(service || '').toUpperCase()} prepared=${prepared} cacheHits=${cacheHits} realCalls=${realCalls}/${realCallLimit} scanned=${candidates.length}`);
    }).catch(err => {
        if (isQueueOverflowError(err)) {
            incrementMetric('preload.droppedQueue');
            logger.info(`[PRELOAD] drop | reason=queue_backlog service=${service}`);
            return;
        }
        logger.warn(`[PRELOAD] Queue error: ${err.message}`);
    });
}
...
async function resolveDebridLink(config, item, showFake, reqHost, meta) {
    try {
        ...
        let streamData = null;
        if (service === 'rd') {
            const directCacheKey = DownloadLinkCache.buildLazyKey(service, runtimeItem, meta);
            streamData = await DownloadLinkCache.get(Cache, directCacheKey, { logger, incrementMetric, scope: 'direct' });
            if (!streamData?.url) {
                streamData = await RD.getStreamLink(apiKey, item.magnet, runtimeItem.season, runtimeItem.episode, item.fileIdx);
                if (streamData?.url) {
                    await DownloadLinkCache.set(Cache, directCacheKey, streamData, { logger, incrementMetric, scope: 'direct' });
                }
            }
        }
...
async function fetchTitleCandidatePool({ type, finalId, tmdbIdLookup, meta, config, dbOnlyMode, sourceModeFlags = null, torrentPipelineEnabled = true, langMode, aggressiveFilter, userTmdbKey, seedResults = [], backgroundRun = false, backgroundReason = 'live' }) {
    ...
    const executionContext = backgroundRun ? 'background' : 'live';
    const disableLiveSources = backgroundRun ? false : flags.useLiveSources !== true;
    const providerModeSignature = buildProviderModeSignature(config, getTorrentProviderNames());
    const titleKey = buildTitleSearchPipelineKey(meta, type, langMode, disableLiveSources, {
        ...(config?.filters || {}),
        sourceMode: flags.sourceMode,
        providerModeSignature
    });
    const rawStreamCacheKey = buildTitleSearchPipelineKey(meta, type, langMode, false, {
        ...(config?.filters || {}),
        sourceMode: 'raw-stream',
        providerModeSignature
    });
    const hotCached = backgroundRun ? null : getTimedCacheValue(titleSearchHotCache, titleKey);
    if (hotCached) {
        logger.info(`[TITLE-QUEUE] Hot cache hit | key=${titleKey} | results=${hotCached.length}`);
        return hotCached;
    }

    const rawCached = (!backgroundRun && flags.bypassProviderCache !== true)
        ? await RawStreamCache.get(rawStreamCacheKey)
        : null;
    if (rawCached) {
        logger.info(`[TITLE-QUEUE] Raw stream cache hit | key=${rawStreamCacheKey} | results=${rawCached.length}`);
        return setTimedCacheValue(titleSearchHotCache, titleKey, rawCached, TITLE_SEARCH_HOT_TTL_MS);
    }

    const inflightPrefix = backgroundRun ? 'title_search_bg' : 'title_search';
    return withSharedPromise(titleSearchInflight, `${inflightPrefix}:${titleKey}`, async () => {
        const cachedAgain = backgroundRun ? null : getTimedCacheValue(titleSearchHotCache, titleKey);
        if (cachedAgain) return cachedAgain;

        return scheduleRequestTask(backgroundRun ? 'title-search-bg' : 'title-search', titleKey, async () => {
            logger.info(`[TITLE-QUEUE] context=${executionContext} reason=${backgroundReason} providerModes=${providerModeSignature}`);
            let dynamicTitles = [];
            try {
            ...
            for (const phase of plan.phases) {
                ...
                if (phase.kind === 'fast') {
                    const remoteMode = getProviderExecutionMode('RemoteIndexer', config, 'both');
                    const externalMode = getProviderExecutionMode('ExternalAddons', config, 'both');
                    const remoteAllowed = providerAllowedInExecutionContext('RemoteIndexer', config, executionContext, 'both');
                    const externalAllowed = providerAllowedInExecutionContext('ExternalAddons', config, executionContext, 'both');
                    const providerSourceAllowed = !disableLiveSources || flags.useProviderCachedOnly;

                    const remoteCacheKey = `${type}:${tmdbIdLookup || finalId}:${meta.season}:${meta.episode}`;
                    const remotePromise = (!remoteAllowed || !providerSourceAllowed)
                        ? Promise.resolve([])
                        : Cache.fetchWithCache('RemoteIndexer', remoteCacheKey, 43200, () =>
                            ...
                            , { group: backgroundRun ? 'provider-background' : 'provider-fast' })
                        , providerCacheOptions);
                    if (!remoteAllowed) logProviderModeSkip('RemoteIndexer', remoteMode, executionContext);
                    ...
                    const externalPromise = (!externalAllowed || !providerSourceAllowed)
                        ? Promise.resolve([])
                        : Cache.fetchWithCache('ExternalAddons', externalCacheKey, 43200, () =>
                            ...
                            , { group: backgroundRun ? 'provider-background' : 'provider-fast' })
                        , providerCacheOptions);
                    if (!externalAllowed) logProviderModeSkip('ExternalAddons', externalMode, executionContext);
                    ...
                    cleanResults = await normalizeCandidateResults([...cleanResults, ...remoteResults, ...externalResults].filter(aggressiveFilter), meta, type);
                    cleanResults = applyConfiguredTorrentFilters(cleanResults, config.filters || {});
                } else if (phase.kind === 'scrape' && phase.querySubset.length > 0 && (flags.useLiveSources || backgroundRun)) {
                    logger.info(`[SCRAPER PLAN] context=${executionContext} phase=${phase.key} lang=${langMode} queries=${phase.querySubset.length} timeout=${scraperTimeout}ms | titleKey=${titleKey}`);
                    ...
                    const scraperFallbackMode = getProviderExecutionMode('ScraperModule', config, 'both');
                    const allowedScrapers = sortedScrapers.filter((scraper) => {
                        if (!scraper?.searchMagnet) return false;
                        const providerName = scraper.name || 'ScraperModule';
                        const mode = getProviderExecutionMode(providerName, config, scraperFallbackMode);
                        const allowed = executionContext === 'background' ? providerAllowsBackground(mode) : providerAllowsLive(mode);
                        if (!allowed) logProviderModeSkip(providerName, mode, executionContext);
                        return allowed;
                    });

                    const allScraperTasks = [];
                    phase.querySubset.forEach((q) => allowedScrapers.forEach((scraper) => {
                        const providerName = scraper.name || 'ScraperModule';
                        const requestScopeKey = `${providerName}:${q}`;
                        allScraperTasks.push(
                            scheduleRequestTask(backgroundRun ? 'scrape-bg' : 'scrape', requestScopeKey, () =>
                                guardedProviderCall(
                                    providerName,
                                    LIMITERS.scraper,
                                    scraperTimeout,
                            ...
                            , { group: backgroundRun ? 'scrape-bg' : 'scrape' })
                        );
                    }));

                    const scrapedResultsRaw = (await Promise.allSettled(allScraperTasks))
                        .flatMap((result) => result.status === 'fulfilled' ? result.value : []);
                    cleanResults = await normalizeCandidateResults([...cleanResults, ...scrapedResultsRaw.filter(aggressiveFilter)], meta, type);
                    cleanResults = applyConfiguredTorrentFilters(cleanResults, config.filters || {});
                    logger.info(`[STATS SCRAPER] phase=${phase.key} total=${cleanResults.length} added=${scrapedResultsRaw.length}`);
                }

                assessmentPool = await normalizeCandidateResults([...seedResults, ...cleanResults].filter(aggressiveFilter), meta, type);
            ...
            await RawStreamCache.set(rawStreamCacheKey, cleanResults).catch(() => false);
...
async function generateStream(type, id, config, userConfStr, reqHost) {
  ...
  return withSharedPromise(streamInflight, inflightKey, async () => {
      ...
      if (torrentPipelineEnabled) {
          cleanResults = await normalizeCandidateResults([...localDbResults, ...networkResults].filter(aggressiveFilter), meta, type);
          ...
          if (configuredDebridService && hasDebridKey) {
              cleanResults = await DebridAvailabilityCache.hydrateItems(Cache, configuredDebridService, cleanResults, { logger });
          }
          ...
          rankedList = applyBitrateRuntimeRanking(rankedList, meta, { logger });
          rankedList = applyTorrentSeedHealthRanking(rankedList, { logger });
          rankedList = applySmartStreamDedup(rankedList, meta, { logger });
      } else {
      ...
      finalStreams = normalizeFinalProxyHeaders(finalStreams, config);
      finalStreams = applyConfiguredStreamFilters(finalStreams, filters);

      scheduleBackgroundTitleScrape({
          type,
          finalId,
          tmdbIdLookup,
          meta,
      ...
      const resultObj = {
          ...
          cacheProfile: clientCache.cacheProfile
      };
