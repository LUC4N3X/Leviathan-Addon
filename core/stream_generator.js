...
const PackResolver = require("./intelligence/pack_intelligence");
...
const { applyTorrentHealthEngine, buildHealthLogLine, persistTorrentHealthSnapshot } = require('./torrent/torrent_health_engine');
...
async function generateStream(type, id, config, userConfStr, reqHost, runtimeContext = {}) {
  ...
  if (isWebEnabled) streamCacheVersionParts.push('webProviderMfpRoute=mfpExtractorHeadersV5|cb01DebugHooks=v2|cb01DomainCache=v1|lazyNoProxy=v1|altadefinizioneNoLazy=v1|altadefinizioneMfpFallback=v1|altadefinizioneAdnKrakenNoRawCdn=v1|altadefinizioneExtractorLabel=v1|cinemacityCccdnNativeProxy=v3');
  ...
  return withSharedPromise(streamInflight, inflightKey, async () => {
      ...
      if (torrentPipelineEnabled) {
          ...
          const torrentHealthBefore = rankedList;
          const torrentHealthPass = applyTorrentHealthEngine(rankedList, {
              stage: 'final-ranked',
              meta,
              type,
              service: configuredDebridService,
              langMode,
              filters,
              strictCache: configuredDebridService === 'tb' && hasDebridKey,
              strictTorboxCache: true,
              dbFirstMin: filters?.torrentHealthDbFirstMin ?? process.env.TORRENT_HEALTH_DB_FIRST_MIN ?? TORBOX_DB_FIRST_REAL_MIN,
              keepLanguageCandidateForMode: (item) => keepLanguageCandidateForMode(item, meta, langMode)
          });
          rankedList = preserveRdStatusList(torrentHealthBefore, torrentHealthPass.results, { logger, stage: 'torrent-health' });
          logger.info(buildHealthLogLine(torrentHealthPass.stats));
          trace.stage('torrent-health', torrentHealthPass.stats);
          persistTorrentHealthSnapshot(rankedList, {
              service: configuredDebridService,
              meta,
              dbHelper,
              logger
          }).catch((error) => logger.warn(`[TORRENT HEALTH] persist failed: ${error?.message || error}`));
      } else {
      ...
      if (!sourceModeFlags.liveOnlyMode) {
          await trace.time('cache-write', () => Cache.cacheStream(cacheKey, resultObj, cachePolicy.localTtl || (finalStreams.length > 0 ? 1800 : EMPTY_STREAM_TTL), {
              ...
              localTtl: cachePolicy.localTtl || (finalStreams.length > 0 ? 1800 : EMPTY_STREAM_TTL),
