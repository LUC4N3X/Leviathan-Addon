...
const { getSourceState } = require('../rd/guards/rd_status_guard');
...
function detectCacheTier(item = {}) {
  ...
  if (item?.isSavedCloud || item?._savedCloud || item?.savedCloud || hints.savedCloud) return 5;
  if (state === 'cached') return 4;
  if (state === 'likely_cached') return 3;
