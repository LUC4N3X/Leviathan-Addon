...
test('episode precision marks Torrentio-style fileIdx + filename as exact', () => {
    const item = {
        hash: 'a'.repeat(40),
        fileIdx: 12,
        behaviorHints: { filename: 'Jujutsu.Kaisen.S02E17.1080p.WEB-DL.mkv' }
    };
    EpisodePrecision.applyEpisodePrecisionToItem(item, { isSeries: true, title: 'Jujutsu Kaisen', season: 2, episode: 17 });
    assert.equal(item._episodeExact, true);
    assert.equal(item._rdEpisodeProof.fileIdx, 12);
});

test('episode precision refuses raw fileIdx when filename proves another episode', () => {
    const item = {
        hash: 'b'.repeat(40),
        fileIdx: 12,
        behaviorHints: { filename: 'Jujutsu.Kaisen.S02E16.1080p.WEB-DL.mkv' }
    };
    EpisodePrecision.applyEpisodePrecisionToItem(item, { isSeries: true, title: 'Jujutsu Kaisen', season: 2, episode: 17 });
    assert.equal(item._episodeExact, undefined);
});
