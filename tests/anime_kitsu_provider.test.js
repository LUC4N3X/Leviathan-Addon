...
test('anime handlers no longer depend on provider_utils bridge', () => {
    ...
    const legacyBridgeRequire = "require('../anime/" + "provider_utils')";

    assert.equal(awHandler.includes(legacyBridgeRequire), false);
    assert.equal(asHandler.includes(legacyBridgeRequire), false);
});
