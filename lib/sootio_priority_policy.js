...
// Sootio-inspired quality policy for Leviathan ranking.
// Conservative: it annotates and softly reranks; it does not hard-drop valid releases.
...
function titleOf(item = {}) {
    return String(item.title || item.name || item.torrent_title || item.filename || '').trim();
}

function sizeOf(item = {}) {
    return Number(item._size || item.sizeBytes || item.size || item.folderSize || 0) || 0;
}
...
function analyzeSootioPriority(item = {}) {
    ...
    const dynamicRange = Array.isArray(details.dynamicRange) && details.dynamicRange.length > 0 ? details.dynamicRange.join('+') : (HDR_RE.test(title) ? 'HDR' : 'SDR');
    ...
    if (codec === 'h265') score += 500;
    else if (codec === 'h264') score += 350;
    if (dynamicRange !== 'SDR') score += 450;
    if (details.flags?.proper) score += 200;
    if (details.flags?.repack || details.flags?.rerip) score -= 250;
    if (AAC_OPUS_RE.test(title)) score -= 950;
    if (JUNK_RELEASE_RE.test(title)) score -= 3500;
    if (item._dbCachedRd === true || item.cached_rd === true || item._tbCached === true) score += 2500;
    if (String(item._rdCacheState || item.rdCacheState || '').toLowerCase() === 'likely_cached') score += 1200;
