...
# on unsupported labels, so we automatically walk down to chrome120.
DEFAULT_IMPERSONATE_CHAIN = ["chrome136", "chrome133", "chrome124", "chrome120"]
...
DEFAULT_CONFIG: Dict[str, str] = {
    ...
    # They are kept here so there is no separate cf_curl_cffi.env file.
...
CHALLENGE_RE = re.compile(
    ...
    r"challenge-platform|turnstile\.cloudflare\.com",
    re.I,
)
...
def normalize_headers(
    ...
    *,
    ...
    if browser_headers:
        ...
        set_header_if_missing(out, "Accept", "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8")
        ...
        set_header_if_missing(out, "sec-ch-ua", f'"Chromium";v="{chrome_major}", "Google Chrome";v="{chrome_major}", "Not.A/Brand";v="99"')
        ...
        if referer:
            ...
            # A same-origin referer often helps providers that reject deep links.
...
def is_usable_proxy_url(value: str) -> bool:
    ...
    if parsed.scheme.lower() not in {"http", "https", "socks4", "socks4a", "socks5", "socks5h"}:
        return False
    # curl_cffi wants a real proxy listener. Kraken /forward?url= endpoints are
    # HTTP fetch endpoints, not CONNECT/SOCKS proxies, so do not use them here.
...
def is_challenge_page(text: str, status: int) -> bool:
    body = str(text or "")
    if int(status or 0) in {403, 429, 503}:
        return True
    return bool(CHALLENGE_RE.search(body[:50000]))
        ...
        for retry_index in range(retry_budget + 1):
            ...
            try:
                headers = normalize_headers(
                    raw_headers,
                    default_user_agent=DEFAULT_CHROME_120_UA,
                    url=args.url,
                ...
                if args.warmup_origin and origin and origin.rstrip("/") != str(args.url).rstrip("/"):
                    try:
                        ...
                        session.get(origin, **warm_kwargs)
                ...
                challenge = is_challenge_page(html, status)
                ...
                attempts.append({"impersonate": impersonate, "retry": retry_index, "status": status, "challenge": challenge, "ms": int((time.time() - attempt_started) * 1000)})
