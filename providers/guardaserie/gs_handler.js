...
async function searchProviderSequential(query) {
  ...
  const ajaxHtml = await smartFetch(ajaxUrl, { isPost: true, body: ajaxBody, ttl: TTL_SEARCH, isPageRequest: false });
  const ajaxRes  = extractSearchResultsFromHtml(ajaxHtml, baseUrl);
  if (ajaxRes.length) return ajaxRes;

  const wpHtml = await smartFetch(`${baseUrl}/?s=${encodeURIComponent(query)}`, { ttl: TTL_SEARCH });
  const wpRes  = extractSearchResultsFromHtml(wpHtml, baseUrl);
  if (wpRes.length) return wpRes;
...
async function _searchGuardaserie(meta, config, season, episode) {
  ...
  for (const result of allResults) {
    if (!/\/serie\//i.test(result.url)) continue;
