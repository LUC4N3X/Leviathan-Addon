...
function shouldTryFallback(errorOrResult) {
    ...
    const status = Number(errorOrResult.status || errorOrResult.statusCode || errorOrResult.response?.status || 0);
...
class ProviderFallbackManager {
    ...
    async runFallbacks({ recipe = {}, context = {}, reason = null, runner }) {
        ...
        const attempts = [];
        const results = [];
        for (const fallback of this.getFallbacks(recipe)) {
            const startedAt = Date.now();
            try {
                const value = await runner(fallback, context, reason);
                const list = Array.isArray(value) ? value : (value ? [value] : []);
                attempts.push({ fallback: fallback.id, ok: true, count: list.length, ms: Date.now() - startedAt });
                results.push(...list);
                if (list.length > 0 && fallback.stopOnHit !== false) break;
            } catch (error) {
                attempts.push({ fallback: fallback.id, ok: false, error: error?.message || String(error), ms: Date.now() - startedAt });
            }
        }
